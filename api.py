...
from makeTreeDir import INPUT_CSVTOCSVHEAT, OUTPUT_CSVTOFCSV, OUTPUT_CSVTOHMGRAPH, OUTPUT_TXTTOCSV, OUTPUT_FCSVTOFGRAPH, BG_MAP, OUTPUT_NFIXTABLE, INPUT_CSVTOCSVHEAT, OUTPUT_SACADE, OUTPUT_EXCEL, main_dir, cdCreate, createTree, clearFolder
from pathlib import Path
import re

SEARCH_DATA_PATH = '/home/lordwaif/documents/usecase_andiara'
...
def execToCsv():
    ...
    bar = tqdm(total=len(txt_finders),
               desc="run_task={}".format("Gerando csv's"))
    for i in txt_finders:
        input = i.absolute()
        output = Path.joinpath(i.parent.parent.absolute(),
                               OUTPUT_TXTTOCSV, i.stem+'.csv')
...
def execFixacao():
    ...
    bar = tqdm(total=len(csv_finders),
               desc="run_task={}".format('gerando fixações'))
    for i in csv_finders:
        input = Path.joinpath(i.parent.parent.absolute(),
                              OUTPUT_TXTTOCSV, i.stem+'.csv')
        output = Path.joinpath(i.parent.parent.absolute(),
                               OUTPUT_CSVTOFCSV, i.stem+'.csv')
...
def execFGraph():
    ...
    csv_finders = list(filter(fill, csv_finders))
    bar = tqdm(total=len(csv_finders),
               desc="run_task={}".format('gerando graficos'))
    for i in csv_finders:
        input = i.absolute()
        output = Path.joinpath(i.parent.parent.absolute(),
                               OUTPUT_FCSVTOFGRAPH, i.stem+'.html')
        ...
        if match:
            ...
            bg_path = os.path.join(
                IMAGE_PATH, IMAGE_NAME_PREFFIX+match.groups()[0]+IMAGE_NAME_SUFFIX)
            ...
            output = Path.joinpath(
                output.parent, output.stem+output.parts[-4]+output.suffix)
            # titulo = output.parts[-4]+' Questão '+BG_MAP[input.parts[-3]][int(match.groups()[0])]
            titulo = output.parts[-4]+' ,' + \
                re.sub('V[ií]deo \d\. ', '', output.stem).replace(
                    '_atv_0_', '')
        cmd = "python3 graficoFixacao.py '"+input.__str__()+"' -o '"+output.__str__() + \
            "'"+bg_command+" -tbg '"+titulo+"'"
        # print(cmd)
        os.popen(cmd=cmd).read()
        bar.update(1)
    ...
...
def compactAll(directory=TEMPORAY_COMPACT_FOLDER):
    ...
    for ind, graph_list in enumerate(graphs):
        bar = tqdm(total=len(graph_list),
                   desc="run_task={}".format('copying...'))
        for i in graph_list:
        ...
        cmd = "tar -zvcf '"+titulo_arquivo[ind] + \
            ".tar.gz' '"+Path(directory).name+"/'"
        import subprocess
        import sys
        t = tqdm(total=len(graph_list),
                 desc="run_task={}".format("comprimindo.."))
...
def execInputHeatMap():
    ...
    csv_finders = list(filter(fill, csv_finders))
    bar = tqdm(total=len(csv_finders), desc="run_task={}".format(
        "Gerando csv's para HeatMap"))
    for i in csv_finders:
        input = i.absolute()
        output = Path.joinpath(i.parent.parent.absolute(),
                               INPUT_CSVTOCSVHEAT, i.stem+'.csv')
...
def execHeatMap():
    ...
    csv_finders = list(filter(fill, csv_finders))
    bar = tqdm(total=len(csv_finders), desc="run_task={}".format(
        "Gerando graficos de HeatMap"))
    for i in csv_finders:
        input = i.absolute()
        output = Path.joinpath(i.parent.parent.absolute(),
                               OUTPUT_CSVTOHMGRAPH, i.stem+'.png')
        match = re.match(re.compile(REGEXP_MAP_WITH_BG), input.stem)
        bg_command = ''
        titulo = GRAPH_TITLE_DEFAULT
        if match:
            ...
            titulo = output.parts[-4]+' ,' + \
                re.sub('V[ií]deo \d\. ', '', output.stem).replace(
                    '_atv_0_', '')
        os.popen("python3 gazeheatplot.py '"+str(input) +
                 "' 1920 1080 -a 0.6 -o '"+str(output)+"'"+bg_command).read()
        bar.update(1)
    ...
    ...
    for i in graph_finders:
        ...
        if match and match_name:
            ...
            for j in range(0, len(lista)):
                ...
                def xl(x): return int(x*proporcao_x)
                def yl(y): return int(y*proporcao_y)
                inf_e = lista[j][0]
                sup_d = lista[j][1]
                df = pd.read_csv(Path.joinpath(input.parent.parent, OUTPUT_CSVTOFCSV,
                                 nome_arq), sep=DEFAULT_SEP_DF, encoding=DEFAULT_ENCODING)
                df = df[df['X'] > xl(inf_e[0])]
                df = df[df['Y'] < yl(inf_e[1])]
                df = df[df['X'] < xl(sup_d[0])]
                df = df[df['Y'] > yl(sup_d[1])]
                fix_per_reg = df
                dados.append([i.parts[-3], re.match(re.compile('.*atv_\d_(.*)_q'),
                             i.stem).groups()[0], q, j+1, fix_per_reg.shape[0]])
    resultado = pd.DataFrame(
        dados, columns=['Saeb', 'Aluno', 'Questao', 'Regiao', 'Quantidade_Fixacoes'])
    resultado.to_csv(os.path.join(PATH_N_FIX_TABLE,
                     'resultados_nfix.csv'), sep=';', index=False)
    ...
...
def fload():
    for i in Path('/home/lordwaif/documents/eye_leo').rglob("*.txt"):
        cmd = "cp '"+i.__str__()+"' '"+Path(SEARCH_DATA_PATH).joinpath(
            main_dir).joinpath(i.parts[-2], 'dados/input_txt').__str__()+"'"
        print(cmd)
        os.popen(cmd)
...
def sacade():
    ...
    for i in csv_finders:
        ...
        r.append(input.parts[-4]+' | '+input.parts[-1] +
                 ' | Sacadas Qtd: '+str(res))
...
def numeros():
    ...
    for i in csv_finders:
        input = i.absolute()
        output = Path.joinpath(i.parent.parent.absolute(),
                               OUTPUT_EXCEL, i.stem+'.xlsx')

        cmd = "python3 countFix.py '"+input.__str__()+"' -o '"+output.__str__()+"'"
        # print(cmd)
